...
from chan.common.enum import BSP_TYPE
...
from .point import CBS_Point
from .config import CBSPointConfig, CPointConfig
...
class CBSPointList(Generic[LINE_TYPE, LINE_LIST_TYPE]):
    ...
    def treat_pz_bsp1(self, seg: CSeg[LINE_TYPE], BSP_CONF: CPointConfig, bi_list: LINE_LIST_TYPE, is_target_bsp):
        ...
        if last_bi.dir != seg.dir:
            return
        if last_bi.is_down() and last_bi._low() > pre_bi._low():  # 创新�?
            return
        if last_bi.is_up() and last_bi._high() < pre_bi._high():  # 创新�?
            return
