...
from chan.common.enum import BI_DIR, BI_TYPE, DATA_FIELD, FX_TYPE, MACD_ALGO
...
class CBi:
    def __init__(self, begin_klc: CKLine, end_klc: CKLine, idx: int, is_sure: bool):
        ...
        self.parent_seg: Optional[CSeg[CBi]] = None  # 在哪个线段里�?
    ...
    def check(self):
        try:
            ...
            raise CChanException(f"{self.idx}:{self.begin_klc[0].time}~{self.end_klc[-1].time}笔的方向和收尾位置不一�?", ErrCode.BI_ERR) from e
    ...
    @make_cache
    def _mid(self):
        return (self._high() + self._low()) / 2  # 笔的中位�?
    ...
    def Cal_MACD_slope(self):
        ...
        end_klu = self.get_end_klu()
        if self.is_up():
            denominator = end_klu.high
            numerator = end_klu.high - begin_klu.low
        else:
            denominator = begin_klu.high
            numerator = begin_klu.high - end_klu.low
        if denominator == 0:
            return 0.0
        return numerator / denominator / (end_klu.idx - begin_klu.idx + 1)
