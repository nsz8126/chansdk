...
from chan.common.util import has_overlap
...
class CZS(Generic[LINE_TYPE]):
    def __init__(self, lst: Optional[List[LINE_TYPE]], is_sure=True):
        # begin/end：永远指�?klu
        # low/high: 中枢的范�?
        # peak_low/peak_high: 中枢所涉及到的笔的最大值，最小�?
        ...
        self.__bi_in: Optional[LINE_TYPE] = None  # 进中枢那一�?
        self.__bi_out: Optional[LINE_TYPE] = None  # 出中枢那一�?

        self.__bi_lst: List[LINE_TYPE] = []  # begin_bi~end_bi之间的笔，在update_zs_in_seg函数中更�?
    ...
    def update_zs_range(self, lst):
        ...
        self.__mid: float = (self.__low + self.__high) / 2  # 中枢的中�?
        self.clean_cache()
    ...
    def is_divergence(self, config: CPointConfig, out_bi=None):
        if not self.end_bi_break(out_bi):  # 最后一笔必须突破中�?
            return False, None
        ...
        if config.divergence_rate > 100:  # 保�?
