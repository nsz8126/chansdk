...
from chan.bi.list import CBiList
from chan.bsp.list import CBSPointList
from chan.config import CChanConfig
from chan.common.enum import KLINE_DIR, SEG_TYPE
from chan.common.exception import CChanException, ErrCode
from chan.seg.seg import CSeg
from chan.seg.config import CSegConfig
from chan.seg.list_comm import CSegListComm
from chan.zs.list import CZSList

from .kline import CKLine
from .unit import CKLine_Unit


def get_seglist_instance(seg_config: CSegConfig, lv) -> CSegListComm:
    if seg_config.seg_algo == "chan":
        from chan.seg.list_chan import CSegListChan
        return CSegListChan(seg_config, lv)
    elif seg_config.seg_algo == "1+1":
        warnings.warn(f"seg_algo={seg_config.seg_algo} is deprecated and no longer maintained.", DeprecationWarning, stacklevel=2)
        from chan.seg.list_dyh import CSegListDYH
        return CSegListDYH(seg_config, lv)
    elif seg_config.seg_algo == "break":
        warnings.warn(f"seg_algo={seg_config.seg_algo} is deprecated and no longer maintained.", DeprecationWarning, stacklevel=2)
        from chan.seg.list_def import CSegListDef
        return CSegListDef(seg_config, lv)
    else:
        raise CChanException(f"unsupport seg algorithm:{seg_config.seg_algo}", ErrCode.PARA_ERROR)
...
class CKLine_List:
    ...
    def cal_seg_and_zs(self):
        if not self.step_calculation:
            if len(self.lst) > 0:  # 防御性检查：避免空列表访�?
        ...
        # 计算买卖�?
        self.seg_bs_point_lst.cal(self.seg_list, self.segseg_list)  # 线段线段买卖�?
    ...
    def add_single_klu(self, klu: CKLine_Unit):
        ...
        if len(self.lst) == 0:
            ...
            if _dir != KLINE_DIR.COMBINE:  # 不需要合并K�?
