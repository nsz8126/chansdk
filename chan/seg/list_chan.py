...
from chan.common.enum import BI_DIR, SEG_TYPE

from .eigen_fx import CEigenFX
from .config import CSegConfig
from .list_comm import CSegListComm
...
class CSegListChan(CSegListComm):
    ...
    def do_init(self):
        ...
        if len(self):
            ...
            if not self.lst[-1].eigen_fx.ele[-1].lst[-1].is_sure:
                # 如果确定线段的分形的第三元素包含不确定笔，也需要重新算，不然线段分形元素的高低点可能不�?
                self.lst.pop()
    ...
    def cal_seg_sure(self, bi_lst: CBiList, begin_idx: int):
        up_eigen = CEigenFX(BI_DIR.UP, lv=self.lv)  # 上升线段下降�?
        down_eigen = CEigenFX(BI_DIR.DOWN, lv=self.lv)  # 下降线段上升�?
    ...
    def treat_fx_eigen(self, fx_eigen, bi_lst: CBiList):
        ...
        if _test in [True, None]:  # None表示反向分型找到尾部也没找到
            is_true = _test is not None  # 濡傛灉鏄甯哥粨鏉?
            if not self.add_new_seg(bi_lst, end_bi_idx, is_sure=is_true and fx_eigen.all_bi_is_sure()):  # 防止第一根线段的方向与首尾值异�?
