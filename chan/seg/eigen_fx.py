...
from chan.common.enum import BI_DIR, FX_TYPE, KLINE_DIR, SEG_TYPE
from chan.common.exception import CChanException, ErrCode
from chan.common.util import revert_bi_dir
...
class CEigenFX:
    ...
    def treat_second_ele(self, bi: CBi) -> bool:
        ...
        if combine_dir != KLINE_DIR.COMBINE:  # 不能合并
            ...
            if (self.is_up() and self.ele[1].high < self.ele[0].high) or \
               (self.is_down() and self.ele[1].low > self.ele[0].low):  # 前两元素不可能成为分�?
                return self.reset()
        return False
    ...
    def add(self, bi: CBi) -> bool:  # 返回是否出现分形
        ...
        if self.ele[0] is None:  # 第一元素
            ...
            raise CChanException(f"特征序列3个都找齐了还没处�?! 当前�?{bi.idx},当前:{str(self)}", ErrCode.SEG_EIGEN_ERR)
    ...
    def reset(self):
        ...
        if self.exclude_included:
            ...
            self.lst = [bi for bi in bi_tmp_list if bi.idx >= ele2_begin_idx]  # 从第二元素开�?
    ...
    def find_revert_fx(self, bi_list: CBiList, begin_idx: int, thred_value: float, break_thred: float):
        COMMON_COMBINE = False  # 是否用普通分形合并规则处�?
        # 如果返回None，表示找到最后了
        first_bi_dir = bi_list[begin_idx].dir  # down则是要找顶分�?
        egien_fx = CEigenFX(revert_bi_dir(first_bi_dir), exclude_included=not COMMON_COMBINE, lv=self.lv)  # 顶分型的话要找上升线�?
        for bi in bi_list[begin_idx::2]:
            ...
            # 这段逻辑删除的原因参�?272，如果有其他badcase，再看怎么统一修复
            #     return False
        return None
